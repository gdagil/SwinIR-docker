...
def main():
    ...
    raw_files_list:list = list(enumerate(sorted(glob.glob(os.path.join(folder, '*')))))
    for idx, path in raw_files_list:
        if path.endswith('.mp4'):
            video_frames_list, FPS, video_name = vc.video_to_images(path)
            video_list.append({
                "idx": idx,
                "path": path,
    ...
    for idx, path in raw_files_list:
        ...
        for item in video_list:
            for img in item["video_frames_list"]:
                if imgname in img:
                    img_path = f'{save_dir}/{imgname}.png'
                    item["upped_imgs"].append(img_path)
...
def setup(args):
    ...
    # 004 grayscale image denoising/ 005 color image denoising
        ...
        save_dir = f'{args.folder_ts}/swinir_{args.task}_noise{args.noise}'
        folder = args.folder_gt
        border = 0
        window_size = 8
    ...
    # 006 JPEG compression artifact reduction
        ...
        save_dir = f'{args.folder_ts}/swinir_{args.task}_jpeg{args.jpeg}'
        folder = args.folder_gt
        border = 0
        window_size = 7
