...
def video_to_images(video_file:str):
    ...
    while True:
        ...
        cv2.imwrite(f"temp/{file_name}-frame_{frame_duration_formatted}.png", frame)
        files_list.append(f"temp/{file_name}-frame_{frame_duration_formatted}.png")
        count += 1
...
def generate_video(files_list:list, path:str, video_file_name:str, FPS:int, save_frames:bool = False):
    ...
    video_name = f'{path}/up_{video_file_name}.mp4'
